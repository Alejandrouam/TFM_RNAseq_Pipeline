...
for (comparison in names(filt_res_list)){
  write.csv2(filt_res_list[[i]], paste0(out_dir_edgeR, comparison, "/", comparison, "_deg.csv"), row.names = FALSE)
  i<- i + 1
}
i=1
for (comparison in names(filt_res_list_deseq)){
  write.csv2(filt_res_list_deseq[[i]], paste0(out_dir_deseq, comparison, "/", comparison, "_deg.csv"), row.names = FALSE)
  i<- i + 1
}
