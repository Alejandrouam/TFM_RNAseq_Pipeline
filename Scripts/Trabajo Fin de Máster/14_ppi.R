...
comparison_list_DESeq2 <- list.files(path = paste0(input, "DESeq2/"), pattern = "_deg.csv", recursive = TRUE, full.names  = TRUE)
comparison_list_edgeR <- list.files(path = paste0(input, "edgeR/"), pattern = "_deg.csv", recursive = TRUE, full.names  = TRUE)
...
for (comparison_file in comparison_list) {
  ...
  comparison <- gsub("_deg.csv", "", basename(comparison_file))
